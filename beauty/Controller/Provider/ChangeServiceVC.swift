//
//  ChangeServiceVC.swift
//  beauty
//
//  Created by cs on 2020/4/8.
...
class ChangeServiceVC: BaseViewController {
    ...
    var providerServiceName: String = ""
    ...
    func initData() {
        if let provider = Provider {
            ...
            mainImageView.kf.setImage(with: URL(string: provider.providerMainImageUrl), placeholder: UIImage(named: "placeholder"))
            
            logoImageView.kf.indicatorType = .activity
            logoImageView.kf.setImage(with: URL(string: provider.providerLogoImageUrl), placeholder: UIImage(named: "placeholder"))
            
            certificationImageView.kf.indicatorType = .activity
            certificationImageView.kf.setImage(with: URL(string: provider.providerCertificationImageUrl), placeholder: UIImage(named: "placeholder"))
            
            if provider.isTravelService == 1 {
                travelServiceSwitchBtn.setOn(true, animated: false)
            } else {
                travelServiceSwitchBtn.setOn(false, animated: false)
            }
            descriptionTextView.text = provider.providerDescription
            tfFacebook.text = provider.facebook_link
            tfInstagram.text = provider.instagram_link
            tfBusinessName.text = provider.business_name
        }
    }
    ...
    func loadCategoryList() {
        ...
        for item in serviceNames {
            ...
            if let provider = Provider {
                providerServiceIds = provider.providerServiceType.components(separatedBy: ",")
            }
            if providerServiceIds.contains(one.index) {
                one.selected = true
                providerServiceName += one.serviceTypeName + ","
            }
            serviceTypeData.append(one)
        }
        
        providerServiceName = String(providerServiceName.dropLast())
        service1Field.placeholder = ""
        lblSelectedServices.text = providerServiceName
...
extension ChangeServiceVC: UITableViewDelegate {
...
extension ChangeServiceVC : UITableViewDataSource {
...
extension ChangeServiceVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
